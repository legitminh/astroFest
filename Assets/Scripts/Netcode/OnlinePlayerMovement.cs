...
public class OnlinePlayerMovement : NetworkBehaviour
{
    ...
    //Local loop to keep in touch, each OBJECT copy is created to they need sync each frame
    void WebUpdate()
    {
    ...
    void WebUpdateSync()
    {
        ...
        {
            //Otherwise then you need to rpc
            SubmitPositionRequestServerRpc();
        }
        print("Sync" + Position.Value);
    }
    ...
    Color standardColor = Color.white;//color without effect | true color
    ...
    void Update()
    {
        //__ADDED__ Only owner can control,
        WebUpdate();
        if (!IsOwner)
        {
            return;
